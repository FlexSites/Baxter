...
var app = angular.module("comedian", [ "ngResource", "ngRoute" ]).config([ "$routeProvider", "$locationProvider", "$httpProvider", function($routeProvider, $locationProvider, $httpProvider) {
    ...
    return $resource("http://testapi.comedian.io/users/:id"), {
        id: "@id"
    };
} ]).factory("Event", [ "$resource", function($resource) {
    return $resource("http://testapi.comedian.io/events/:id", {
        id: "@id"
    });
} ]).factory("Venue", [ "$resource", function($resource) {
    return $resource("http://testapi.comedian.io/venues/:id", {
        id: "@id"
    });
} ]).factory("Medium", [ "$resource", function($resource) {
    return $resource("http://testapi.comedian.io/media/:id", {
        id: "@id"
    });
} ]).factory("Entertainer", [ "$resource", function($resource) {
    return $resource("http://testapi.comedian.io/entertainers/53d1db2b562cdbef37fe0a48");
} ]).controller("ListController", [ "$http", "$scope", "$routeParams", "items", function($http, $scope, $routeParams, items) {
    $http.get("http://testapi.comedian.io/auth").success(function() {
    ...
    return {
        getUser: function() {
            ...
            if (_user.loggedIn === true) {
                ...
                $http.get("http://testapi.comedian.io/auth").success(function(user) {
